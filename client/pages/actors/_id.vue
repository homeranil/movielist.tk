<template>
  <div>
    <div v-if="actor" class="border-b border-gray-800">
      <div class="container mx-auto px-4 py-10 flex flex-col md:flex-row">
        <div class="flex-none">
          <img :src="actor.image" alt="" class="w-76" />
          <ul class="flex items-center mt-4 justify-center">
            <li v-if="actor.external_ids.facebook_id" class="mx-4">
              <a
            ...
            <li v-if="actor.external_ids['instagram_id']" class="mx-4">
              <a
            ...
            <li v-if="actor.external_ids['twitter_id']" class="mx-4">
              <a
            ...
            <li v-if="actor.homepage" class="mx-4">
        ...
        <div class="md:mr-24">
          ...
          <div class="text-gray-400 text-sm text-right" :class="dir">
          ...
          <div class="flex flex-wrap items-center text-gray-400 text-sm mt-1">
            ...
            <span class="mr-2"> {{ birthday }} ({{ age }}) </span>
          </div>

          <p class="text-gray-300 mt-8" :class="dir">
            {{ actor.biography }}
          </p>

          <h4 class="font-semibold mt-12">
            <span v-if="actor.gender === 1">
              השתתפה ב
            </span>
            <span v-else>השתתף ב</span>
          </h4>
          <div
            class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-8"
          >
            <div v-for="item in actor.know_for" :key="item.id" class="mt-4">
              <nuxt-link
                :to="{
                  name: item.media_type === 'tv' ? 'tv-id' : 'movies-id',
                  params: { id: item.id },
                }"
              >
                <img
              ...
              <nuxt-link
                :to="{ name: 'movies-id', params: { id: item.id } }"
    ...
    <div class="credits border-b border-gray-800">
      <div class="container mx-auto px-4 py-10">
        <h2 class="text-4xl font-semibold">קרדיטים</h2>
        <ul class="list-disc leading-loose pr-5 mt-8">
          <li v-for="(credit, index) in actor.credits" :key="index">
            {{
              credit.release_date
            ...
            <nuxt-link
              :to="{
                name: credit.media_type === 'tv' ? 'tv-id' : 'movies-id',
                params: { id: credit.id },
              }"
...
export default {
  async asyncData({ $axios, params }) {
    const actor = await $axios.$get(`/actors/${params.id}`)
  ...
  computed: {
    dir() {
      return this.actor.heb ? 'rtl' : 'ltr'
    },
    birthday() {
      return this.$moment(this.actor.birthday).format('MMM DD, YYYY')
    },
    age() {
      return this.$moment().diff(this.actor.birthday, 'years')
    },
  },
  methods: {},
  head() {
